...
void sub_802DB6C(u8 taskId)
{
    ...
    else
    {
        ...
        s16 r10 = gTasks[taskId].data[1];
        u8 r7 = gTasks[taskId].data[2];
        s16 r4 = sub_8045C78(r7, gHealthboxSpriteIds[r7], 1, 0);
        sub_8043DFC(gHealthboxSpriteIds[r7]);
        if (r4 == -1)
        {
            u8 r4;
            s32 sp4; //should be u32, but s32 matches
            u16 r0;
            s32 sp0; //same here

            m4aSongNumStop(SE_EXP);
            r4 = GetMonData(&gPlayerParty[r9], MON_DATA_LEVEL);
            sp4 = GetMonData(&gPlayerParty[r9], MON_DATA_EXP);
            r0 = GetMonData(&gPlayerParty[r9], MON_DATA_SPECIES);
            sp0 = gExperienceTables[gBaseStats[r0].growthRate][r4 + 1];
            if (sp4 + r10 >= sp0)
            {
                u8 r5;

                SetMonData(&gPlayerParty[r9], MON_DATA_EXP, &sp0);
                CalculateMonStats(&gPlayerParty[r9]);
            ...
            else
            {
                //u32 asdf = sp4 + r10;
                sp4 += r10;
                SetMonData(&gPlayerParty[r9], MON_DATA_EXP, (u8 *)&sp4);
