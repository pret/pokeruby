...
static void sub_8047254(struct Sprite *sprite)
{
    ...
    if (sprite->data0 > 15)
    {
        sprite->data0 = 0;
        if (IsDoubleBattle() && ewram17840.unk9_0
         && sprite->data6 == battle_get_side_with_given_state(3))
            sprite->callback = sub_8047230;
        else
            sprite->callback = sub_8046C78;
    }
...
{
    struct Sprite *sprite = &gSprites[gUnknown_03004340[a]];
    ...
    if (battle_get_per_side_status(a) == 2)
...
void sub_8047858(u8 a)
{
    ...
    gSprites[spriteId].data1 = gUnknown_03004340[a];
...
{
    if (battle_side_get_owner(a) == 0)
        return GetMonData(&gPlayerParty[gUnknown_02024A6A[a]], MON_DATA_POKEBALL);
    else
        return GetMonData(&gEnemyParty[gUnknown_02024A6A[a]], MON_DATA_POKEBALL);
}
